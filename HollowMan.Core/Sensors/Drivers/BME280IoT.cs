    ...
    public class BME280IoT : Sensor, IDisposable
    {
        ...
        public override ISensorSample TakeReading(WeatherObservation observation)
        {
            ...
            if (result.Humidity.HasValue && result.Pressure.HasValue && result.Temperature.HasValue)
            {
                ...
                double absHumidity = WeatherHelper.CalculateAbsoluteHumidity(rawTemperature, humidity).GramsPerCubicMeter;
                double dewPoint = WeatherHelper.CalculateDewPoint(rawTemperature, humidity).DegreesCelsius;
                double heatIndex = WeatherHelper.CalculateHeatIndex(rawTemperature, humidity).DegreesCelsius;
                double vapourPressure = WeatherHelper.CalculateActualVaporPressure(rawTemperature, humidity).Hectopascals;
                double barometricPressure = WeatherHelper.CalculateBarometricPressure(pressure, rawTemperature, actualAltitude, humidity).Hectopascals;
                double vapourPressureOverIce = WeatherHelper.CalculateSaturatedVaporPressureOverIce(rawTemperature).Hectopascals;
                double vapourPressureOverWater = WeatherHelper.CalculateSaturatedVaporPressureOverWater(rawTemperature).Hectopascals;
                double seaLevelPressure = WeatherHelper.CalculateSeaLevelPressure(pressure, actualAltitude, rawTemperature).Hectopascals;
